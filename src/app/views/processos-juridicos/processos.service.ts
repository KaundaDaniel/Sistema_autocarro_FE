...
export class ProcessosService {
  ...
  public listOfProcessos(filters: any) {
    return this.http.get(
      `${this.httpService.base_url}/processos/listagem?search=${filters.search}&page=${filters.pagination.page}&perPage=${filters.pagination.perPage}&start_date=${filters.start_date}&end_date=${filters.end_date}`,
  ...
  createOrEdit(productForm: any) {
    const url =
      productForm.getRawValue().id == null
        ? `${this.httpService.base_url}/processos/create`
        : `${this.httpService.base_url}/processos/update/` +
          productForm.getRawValue().id;

    return this.http.post(url, productForm.value, {
      headers: this.authService.getHeaders(),
    });
  }
