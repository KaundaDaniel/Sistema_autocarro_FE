...
export class CreateOrEditUsersComponent implements OnInit {
  ...
  createOrEdit() {
    ...
    this.loading = true;
    const url = this.userForm.getRawValue().id == null ? `${this.httpService.base_url}/autenticacao/register` : `${this.httpService.base_url}/users/update/` + this.userForm.getRawValue().id

    this.http.post(url, this.userForm.value, { headers: this.authService.headers })
      .subscribe(res => {
